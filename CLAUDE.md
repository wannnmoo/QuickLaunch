...
- **分组（Stack）**：`isGroup` 条目点击展开面板而不启动；成员用 `groupId` 归属（**扁平模型，不嵌套**——桌面扫描/缺失清理/持久化全部沿用原逻辑）。右键图标「新建分组」创建空组并横向滚动到末尾；分组图标默认渲染**组内前 4 个非分隔线成员的缩略拼图**（0 个成员回退 2×2 网格图标、1 个放大单图、用户换过图标则用自定义图标），右下角 `.dock-badge` 显示成员数。拖到分组图标上即归组（插到该组现有成员之后），从面板拖到 Dock 条内即移出，删除分组=解散（成员回顶层、保留相对位置）；编辑表单对分组只留名称 + 图标。⚠️ **归组时必须抹掉顶层专属标记（v1.13.1 修）**：写的是 `{ ...item, groupId }` 再 `delete member.isFolder / member.specialType`——否则把「此电脑」拖进分组时 `specialType: 'this-pc'` 会跟着进组，这个「分组成员」在面板里仍被当成系统位置渲染（标签被改写成「此电脑 · 可用 …」、图标上还挂一条用量细条），而它的 `targetPath` 是 `shell:` 命令、语义上根本不是普通成员
  - ⚠️ **缩略图是「层叠卡片」而不是 2×2 网格（v1.13.2 重做，改这块必看）**。徽标 `.dock-badge` 是 16×16 锚在图标框右下角（占 `(44,44)..(60,60)`），所以**第 4 张卡不能占满右下角**。旧版是 54×54 铺满图标框的 2×2 网格，4 成员时第 4 张正好落在那个角上 —— 实测被盖住 **25%**（169/676 px²），看起来就像组里少了一块（用户反馈的正是这个）；3 个成员时第二行被居中所以碰不到，**这个 bug 只在 ≥4 个成员时出现**
  - 现在的几何：4 张 22px 卡片走**向右下阶梯**（`(2,2) / (22,0) / (0,22) / (18,18)`），第 4 张右下角距徽标 `(44,44)` 留 3px，**实测重叠 0 px²**（1/2/3/4/8 五种成员数全覆盖）。成员不足 4 个时位置固定不重排（2 个是「左上+右上」、3 个是「上排两个+左下」），看起来仍是同一叠
  - 卡片之间必须有投影（`box-shadow: 0 1px 2px`）分开，否则同色系图标会糊成一片
  - 试过的其他方案与结论：**纯旋转堆叠**（4 张 32px 各转 ±3°/±8°）观感最像 macOS 文件夹，但 44px 下四张糊成一坨、认不出成员，弃用；**只显示 3 张更大的阶梯**辨识度好但少一张成员
  - 🚫🚫 **定位方式试错四轮，两条弯路都别再走**：
    1. `margin` / 负 margin **无效** —— `.dock-icon-wrap` 是 `display:flex` + `align-items/justify-content: center`，flex 的居中断言会盖掉 margin（实测负 margin 完全没生效）
    2. 只写 `position:absolute; top:0; left:0` **也无效** —— 绝对定位只把盒子移出文档流，**并不豁免父级的 flex 对齐**：实测 `offsetLeft/offsetTop` 仍是 `9/9`（被居中）。正解是 `position:absolute` + **四向 inset + `margin:auto`** 把盒子钉到指定角落
  - 教训：**在 flex 容器里给绝对子元素定位时，`top/left` 不一定说了算**，必须实测 `offsetLeft/offsetTop` 或 `getBoundingClientRect()` —— 这处我连续推错三次，每次都被探针打回来。另外「1px 余量」在亚像素布局下会退化成实际重叠，**余量要给够，别卡着算**
  - 回归断言见 `regression.cjs` 第 8 组（`多成员分组的徽标不再盖住缩略图`，容差 8px²）；视觉对比脚本 `.dsh-vision-toolkit/probe/stack-variants.cjs`（一次渲染 6 种候选并输出 1x 截图）
  - ⚠️ 探针踩坑：`e(id, name, extra)` 这类 fixture 辅助函数的**第三参才是附加字段**，写成 `e(901,'M1', G)` 会得到 `group: 'M1'` 这种废字段（实测导致徽标显示 1、第 8 组断言永远失败）。改 fixture 后**先确认读回来的数据里字段真的在**，别急着怀疑应用代码
