...
- **分组（Stack）**：`isGroup` 条目点击展开面板而不启动；成员用 `groupId` 归属（**扁平模型，不嵌套**——桌面扫描/缺失清理/持久化全部沿用原逻辑）。右键图标「新建分组」创建空组并横向滚动到末尾；分组图标默认渲染**组内前 4 个非分隔线成员的缩略拼图**（0 个成员回退 2×2 网格图标、1 个放大单图、用户换过图标则用自定义图标），右下角 `.dock-badge` 显示成员数。拖到分组图标上即归组（插到该组现有成员之后），从面板拖到 Dock 条内即移出，删除分组=解散（成员回顶层、保留相对位置）；编辑表单对分组只留名称 + 图标。⚠️ **归组时必须抹掉顶层专属标记（v1.13.1 修）**：写的是 `{ ...item, groupId }` 再 `delete member.isFolder / member.specialType`——否则把「此电脑」拖进分组时 `specialType: 'this-pc'` 会跟着进组，这个「分组成员」在面板里仍被当成系统位置渲染（标签被改写成「此电脑 · 可用 …」、图标上还挂一条用量细条），而它的 `targetPath` 是 `shell:` 命令、语义上根本不是普通成员
  - ⚠️ **缩略拼图必须给右下角的数量徽标让位（v1.13.2 修，改这块必看）**。徽标 `.dock-badge` 是 16×16 锚在图标框右下角（`right/bottom: 0`，占 `(44,44)..(60,60)`），而原来的拼图是 54×54 铺满图标框 —— **4 个成员时第 4 张图正好落在右下角，实测徽标盖住那张图的 25%**（重叠 169/676 px²），4 个成员的分组看起来就像少了一块。3 个成员时第二行单个居中、碰不到，所以这个 bug 只在 **≥4 个成员**时出现
  - 修法：拼图缩到 2×2 = `19 + 2 + 19 = 40`，钉在图标框 `(4,4)..(44,44)`，与徽标**恰好相接不重叠**（实测 0 px²，5 种成员数全覆盖）。徽标另加 `0 0 0 1.5px var(--label-pill-bg)` 外描边 —— 它可能压在任意颜色的图标上，纯色药丸贴上去边界会糊
  - 🚫🚫 **定位方式试错四轮才定，三条弯路都别再走**：
    ...
    2. 只写 `position:absolute; top:0; left:0` **也无效**：绝对定位只把盒子移出文档流，**并不豁免父级的 flex 对齐** —— 实测 `offsetLeft/offsetTop` 仍是 `9/9`（被居中），网格右下角照样压着徽标
    3. 正解是 `position:absolute` + **四向 inset + `margin:auto`**（本项目用 `inset:0` + `margin:4px auto auto 4px` + `align-self/justify-self: flex-start`）把盒子钉在指定角落
  - 教训：**在 flex 容器里定位绝对子元素时，`top/left` 不一定说了算** —— 一定要**实测 `offsetLeft/offsetTop` 或 `getBoundingClientRect()`**，别靠"我写了 top:0 所以它在左上角"这种推断（这一处我连续推错三次，每次都被探针打回来）
  - 回归断言见 `regression.cjs` 第 8 组（`多成员分组的徽标不再盖住缩略图`，容差 8px²）
